...
function generateCards(data) {
  ...
  for (const section in data) {
    ...
    const capitalizedSection =
      section.charAt(0).toUpperCase() + section.slice(1); // Capitalize first letter of section
...
function createHTMLForBusiness(business) {
  return `
    <table width="100%" height="auto" background="${
      business.image_url
        ...
        <tr>
            <td valign="bottom" style="padding: 15px; background-color: rgba(0, 0, 0, 0.5);">
                <p style="font-size: 1.2rem; color: #ffffff; margin: 0;"><b>${
                  business.name
                }</b></p>
                <p style="color: #ffffff; margin: 5px 0 0;"><b>Rating:</b> ${
                  business.rating
                }</p>
                <p style="color: #ffffff; margin: 5px 0 0;"><b>Review Count:</b> ${
        ...
        <tr>
            <td style="padding: 15px; background-color: #f9f9f9;">
                <p><b>Categories:</b> ${business.categories
                  .map((category) => category.title)
                  .join(", ")}</p>
                <p style="font-size: 0.9rem; color: #555; margin-top: 10px;"><b>Address:</b> ${business.location.display_address.join(
                  ", "
                )}</p>
                <p><b>Phone:</b> ${business.display_phone}</p>
                <p style="margin-top: 10px;"><a href="${
                  business.url
                }" target="_blank" style="text-decoration: none; color: #007bff;">Visit Yelp Page</a></p>
            </td>
        </tr>
    </table>
    `;
